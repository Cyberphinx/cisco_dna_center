...
"""
    This code snippet will run execute operational commands across your entire
    network using Cisco DNA Center
    Command Runner APIs
"""
...
def get_auth_token():
    ...
    try:
        ...
        resp = requests.post(
            url, auth=HTTPBasicAuth(DNAC_USER, DNAC_PASS),
            headers=hdr,
            verify=False
        )
...
def initiate_cmd_runner(token):
    ...
    url = "https://{}/api/v1/network-device-poller/cli/read-request".format(
        DNAC_URL)
    header = {'content-type': 'application/json', 'x-auth-token': token}
    logging.captureWarnings(True)
    response = requests.post(
        url, data=json.dumps(param), headers=header, verify=False
    )
